...
#define WNB_STYPE_WAKEUP_STA           0x12
...
struct wnb_mgmt_wakeup {
    struct wnb_mgmt hdr;
    uint8  wk_reason, p1, p2, p3;
    uint8  sta[6];
};
...
#endif
#ifndef WNB_SSID_FUZZY_LEN
#define WNB_SSID_FUZZY_LEN (3)
#endif
...
int8 wnb_set_freq(struct wireless_nb *wnb, uint8 chan_idx);
...
int32 wnb_wakeup_sta(struct wireless_nb *wnb, uint8 *addr, uint8 wk_reason);
...
void wnb_data_tx_prepare(struct wireless_nb *wnb, struct sk_buff *skb, wnb_forward_type forward);
...
int32 wnb_rx_wakeup_frame(struct wireless_nb *wnb, uint8 *data, uint32 len);
int32 wnb_send_wakeup_frame(struct wireless_nb *wnb, struct wnb_sta *sta, uint8 *addr, int8 wk_reason);
...
void wnb_roaming_reset(struct wireless_nb *wnb);
