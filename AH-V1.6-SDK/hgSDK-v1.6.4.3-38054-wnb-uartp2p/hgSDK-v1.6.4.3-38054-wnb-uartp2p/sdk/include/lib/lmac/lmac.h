...
struct dsleep_psdata{
    ...
    uint8               ssid[32], psk[32], bssid[6], paired_addr[6];
...
struct lmac_init_param{
    ...
    uint32 uart_tx_io : 2;//0 for pa13, 1 for pa11, 2 for pa31
};
...
void lmac_sta_info_statics_print(void *p, uint8 print_level);
void lmac_transceive_statics(uint8 print_level);
...
#define lmac_set_sleep_roaming(ops, en, rssi)       ops->setcfg(ops, LMAC_SETCFG_SET_SLEEP_ROAMING, en, rssi)
#ifdef __cplusplus
}
#endif
