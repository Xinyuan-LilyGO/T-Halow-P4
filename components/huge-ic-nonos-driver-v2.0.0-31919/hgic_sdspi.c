...
static int sd_io_rw_extended(void *priv, const SDIO_CMD *cmd53, unsigned char *data)
{
        ...
        if (rw_flag)
        {
            ...
            { // ---------------------------------blocks write
                while (len--)
                {
                    ...
                    if (err)
                    {
                        printf("[log debug]->[%s][%u line]: err: %d\n", __FILE__, __LINE__, err);
                    }
    ...
    if (err)
    {
        printf("[log debug]->[%s][%u line]: sd_io_rw_extended fail (error code: %d)\n", __FILE__, __LINE__, err);
        resp = RET_ERR;
    }
    return resp;
}
...
int hgic_sdspi_data_ready(void *priv)
{
    ...
    if (resp == RET_OK && (data & 0x02))
    {
        ...
        {
            printf("[log debug]->[%s][%u line]: sd_read_byte fail (resp = %d, data = %d)\n", __FILE__, __LINE__, resp, data);
        }
    }
    else
    {
...
int hgic_sdspi_read(void *priv, unsigned char *buf, unsigned int len, int flags)
{
    ...
    if (resp == RET_ERR)
    {
        hgic_sdspi_abort(priv);
        hgic_sdspi_init(priv);
        printf("[log debug]->[%s][%u line]: hgic_sdspi_data_ready fail\n", __FILE__, __LINE__);
        return -1;
    }
    else if (resp > 0)
    {
        ...
        {
            printf("[log debug]->[%s][%u line]: hgic_sdspi_get_datalen fail\n", __FILE__, __LINE__);
            hgic_sdspi_abort(priv);
            hgic_sdspi_init(priv);
            return 0;
        }
        ...
        if (data_len > len)
        {
            printf("[log debug]->[%s][%u line]: data_len > len\n", __FILE__, __LINE__);
            hgic_sdspi_abort(priv);
            return -1;
        }
        ...
        if (data_len == 0)
        {
            printf("[log debug]->[%s][%u line]: data_len == 0\n", __FILE__, __LINE__);
        }

        resp = hgic_sdspi_readdata(priv, buf, data_len);
        if (resp == RET_ERR)
        {
            printf("[log debug]->[%s][%u line]: hgic_sdspi_readdata fail\n", __FILE__, __LINE__);
            hgic_sdspi_abort(priv);
            hgic_sdspi_init(priv);
            return -1;
        }
    ...
    {
        printf("[log debug]->[%s][%u line]: hgic_sdspi_data_ready fail (error code: %d)\n", __FILE__, __LINE__, resp);
    }
...
int hgic_sdspi_write(void *priv, unsigned char *data, unsigned int len)
{
    ...
    if (resp == RET_ERR)
    {
        printf("[log debug]->[%s][%u line]: hgic_sdspi_write fail hgic_sdspi_init start\n", __FILE__, __LINE__);
